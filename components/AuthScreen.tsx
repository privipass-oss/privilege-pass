...
export const AuthScreen: React.FC<AuthScreenProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (foundPartner) {
        if (mode === 'REGISTER') {
          alert("Este email ja e de um parceiro. Faca login.");
          setMode('LOGIN');
          setLoading(false);
          return;
        }
      ...
      if (mode === 'LOGIN') {
        ...
        if (foundCustomer) {
          ...
          alert("Cliente nao encontrado. Verifique o e-mail ou cadastre-se.");
        }
      } else {
        // Register Mode
        const exists = customers.find(c => c.email.toLowerCase() === inputEmail);
        if(exists) {
          alert("Este e-mail ja esta cadastrado. Faca login.");
          setMode('LOGIN');
          setLoading(false);
          return;
        }
        ...
        console.log('[DEBUG] Attempting to insert customer into Supabase...');
        ...
        try {
          ...
          console.log('[SUCCESS] Customer inserted successfully:', data);
        } catch (supabaseError) {
          console.error('[EXCEPTION] Supabase insert threw exception:', supabaseError);
      ...
      console.error('[ERROR] General error in handleSubmit:', generalError);
      ...
      <div className="w-full max-w-md bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative z-10 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-zinc-600 text-right">Minimo 6 caracteres</p>
          </div>
          <button 
            type="submit" 
            disabled={loading}
