...
export const Settings: React.FC<SettingsProps> = ({ 
  ...
  const handleChangePasswordSubmit = () => {
      ...
      // Here we would validate current password if backend existed
  ...
  return (
    <div className="p-8 animate-fade-in max-w-6xl mx-auto pb-20">
                ...
                <div className="flex flex-col md:flex-row items-start gap-8">
                    <div className="relative group cursor-pointer mx-auto md:mx-0">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                            <label htmlFor="avatar-upload" className="cursor-pointer flex flex-col items-center text-xs text-white font-medium">
                  ...
                  <div className="grid gap-4">
                      ...
                      {staff.map(member => (
                          <GlassCard key={member.id} className="flex items-center justify-between" noPadding>
                              ...
                              <div className="p-4">
                                  <button onClick={() => onRemoveStaff && onRemoveStaff(member.id)} className="p-2 text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors">
                                      <Trash2 size={16} />
                                  </button>
                              </div>
                          </GlassCard>
      ...
      {/* Add Staff Modal */}
      {isStaffModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setIsStaffModalOpen(false)}>
              <div className="bg-zinc-900 border border-white/10 rounded-2xl p-8 w-full max-w-md" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-6">
                      <h3 className="text-xl font-bold text-white">Novo Membro</h3>
                      <button onClick={() => setIsStaffModalOpen(false)}><X className="text-zinc-500 hover:text-white" /></button>
                  </div>
                  <div className="space-y-4">
      ...
      {isPasswordModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setIsPasswordModalOpen(false)}>
              <div className="bg-zinc-900 border border-white/10 rounded-2xl p-8 w-full max-w-md" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-6">
